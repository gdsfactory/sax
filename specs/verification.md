...
`JUPYTER_PATH="$PREFIX/share/jupyter" .venv/bin/python -m pytest src/tests -q -rs`:
**419 passed, 1 xfailed, 34 warnings in 29.81 seconds**. Non-notebook subset:
**415 passed, 1 xfailed**. `just smoke`: **6 passed**, 4.33s wall-clock. The single
xfail records that native input without direction cannot feed the `forward`
backend; legacy direction is preserved and legacy forward tests pass. `uv lock`
and `uv lock --check` pass after declaring `kfnetlist>=0.3.0,<0.4.0` with a
`python_version >= '3.12'` marker. The lockfile was regenerated by uv (large
formatting churn). Cross-platform installs were not rerun. Public transforms
