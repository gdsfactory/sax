...
`JUPYTER_PATH="$PREFIX/share/jupyter" .venv/bin/python -m pytest src/tests -q -rs`:
**416 passed, 1 xfailed, 34 warnings in 29.89 seconds**. Non-notebook subset:
**412 passed, 1 xfailed**. `just smoke`: **6 passed**, 5.65s wall-clock. The single
...
formatting churn). Cross-platform installs were not rerun. See the
