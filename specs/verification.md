...
`src/tests/test_native_kfnetlist.py`. Circuit construction runs on native
kfnetlist objects when kfnetlist is importable; legacy dictionaries and
`.pic.yml` are adapted into native first, and `get_required_circuit_models` uses
the native resolver. Without kfnetlist (Python 3.11 minimal install) SAX falls
